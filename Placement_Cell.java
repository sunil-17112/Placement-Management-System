...
public class Placement_Cell {
	...
	private void go_to_tech_round(Company c) {
		...
		for(int i=0;i<student.size();i++) {
			for(int j=0;j<c.get_course().length;j++) {
				if(c.get_course()[j].toUpperCase().equals(student.get(i).get_branch().toUpperCase())&&(student.get(i).get_status().toUpperCase().equals("NOT PLACED"))) {
					c.get_eligible_student() .add(student.get(i));
		...
		for(int i=0;i<c.get_eligible_student().size();i++) {
			System.out.print("Enter the tech marks for roll no  " + c.get_eligible_student().get(i).get_roll_no() + " : ");
			...
			c.get_tech_marks().add(t);
			c.get_temp_select().add(false);
			System.out.println("");
			
		}
		for(int i=0;i<c.get_eligible_student().size();i++) {
			c.get_eligible_student().get(i).get_eligible_company().add(c.get_name());
			c.get_eligible_student().get(i).get_tech_score().add(c.get_tech_marks().get(i));
		}
		
		int i=c.get_eligible_student().size();
		if(c.get_req()>=i) 
		{
			for(int j=0;j<c.get_eligible_student().size();j++) {
				c.get_selected_student().add(c.get_eligible_student().get(j));
				c.get_temp_select().set(i, true);
		...
		else 
		{
			...
			while(j!=c.get_req()) {
				...
				for(int k=0;k<i;k++) {
					if(max==-1) 
					{
						if(c.get_temp_select().get(k)==false)
						{
							max=k;
						}
					}
					...
					{
						if(c.get_tech_marks().get(k)>c.get_tech_marks().get(max)) 
						{
							if(c.get_temp_select().get(k)==false) 
							{
								max=k;
						...
						else if(c.get_tech_marks().get(k)==c.get_tech_marks().get(max))
						{
							if(c.get_temp_select().get(k)==false&&c.get_eligible_student().get(k).get_cgpa()>c.get_eligible_student().get(max).get_cgpa()) 
							{
								max=k;
							}
						}
				...
				c.get_selected_student().add(c.get_eligible_student().get(max));
				c.get_temp_select().set(max, true);
				j++;
	...
	public void selected_student(String com) {
		...
		for(int i=0;i<n;i++) {
			if(company.get(i).get_name().toUpperCase().equals(com.toUpperCase())) {
				if(company.get(i).get_selected_student().size()!=0) {
					System.out.println("Roll_no of selected students");
					for(int j=0;j<company.get(i).get_selected_student().size();j++) {
						company.get(i).get_selected_student().get(j).set_status("Placed");
						company.get(i).get_selected_student().get(j).get_company_offer().add(company.get(i).get_name());
						System.out.println(company.get(i).get_selected_student().get(j).get_roll_no());
					}
	...
	public void display_student_applied_companies(int roll) {
		for(int i=0;i<student.size();i++) {
			if(student.get(i).get_roll_no()==roll) {
					...
					for(int j=0;j<student.get(i).get_eligible_company().size();j++) {
						System.out.println(student.get(i).get_eligible_company().get(j) + "  " + student.get(i).get_tech_score().get(j));
					}
			}
		}
	}
	...
	public static void main(String[] args){
		...
		if(n==0) {
			System.out.println("---No student is register for Placement ---");
