...
public class Placement_Cell {
	int exit=0;
	private int prog_count=2017000;
	...
	public void add_company(String name,String[] a,int req) {
		...
		if(c.get_req()>=i) {
			...
			for(int j=0;j<c.eligible_student.size();j++) {
				...
				//c.eligible_student.get(j).set_status("Placed");
			...
			while(j!=c.get_req()) {
				...
				//c.eligible_student.get(max).set_status("Placed");
				//System.out.println(c.eligible_student.get(max).get_cgpa());
				j++;
	...
	public void remove_student() {
		int f=0;
		for(int i=0;i<student.size();i++) {
			if(student.get(i).get_status().toUpperCase().equals("PLACED")&&student.get(i).Is_selected()==true) {
	...
	public void display_unplaced_students() {
		int stud=0;
		for(int i=0;i<student.size();i++) {
			if(student.get(i).get_status().toUpperCase().equals("NOT PLACED")||(student.get(i).get_status().toUpperCase().equals("PLACED")&&student.get(i).Is_selected()==false)) {
				stud++;
			}
		}
			...
			if(company.get(i).get_name().toUpperCase().equals(com.toUpperCase())) {
				if(company.get(i).select_student.size()!=0) {
					...
					for(int j=0;j<company.get(i).select_student.size();j++) {
						company.get(i).select_student.get(j).set_selected(true);
					...
					System.out.println("NOT finalize shortlist");
				}
			}
		}
	...
	public void display_student_detail(int roll) {
		for(int i=0;i<student.size();i++) {
			if(student.get(i).get_roll_no()==roll) {
				...
				if(student.get(i).get_status().toUpperCase().equals("PLACED")&&student.get(i).Is_selected()==true) {
					System.out.println("PLACED IN " + student.get(i).find_placed_company());
				}
				else {
	...
	public void display_student_applied_companies(int roll) {
		for(int i=0;i<student.size();i++) {
			if(student.get(i).get_roll_no()==roll) {
					...
					student.get(i).find_eligible_company();
			}
		}
	}
	...
	public static void main(String[] args) {
		...
		Placement_Cell main=new Placement_Cell();
